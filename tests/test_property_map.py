...
def test_property_map():
    ...
    prop_map1 = PropertyMap(
        int_map, np.vectorize(
...
def test_materials_property_map():
    ...
    prop_map1 = MaterialPropertyMap(
        int_map, part_materials1, mat_lib, 'electronMass')
    assert prop_map1.get_part((1., 2.)) == 1
    assert np.all(prop_map1.get_part(-np.ones((2, 3))) == 0)
    assert prop_map1((1., 2.)) == mat_lib['GaSb']['electronMass']
    assert np.all(prop_map1(-np.ones((2, 3))) ==
                  mat_lib['InAs']['electronMass'])
    ...
    assert np.all(prop_map2(-np.ones((2, 3))) ==
                  mat_lib.find('InAs', 'eV')['directBandGap'])
