...
def generate_2d_density_plot(generic_task, filename, dask_client=None):
    r"""
    This function takes as input a 2D ThomasFermi or SchrodingerPoisson task,
    and a filename. It then saves all of the data relevant to this plot.
    """
...
def _plot_2d_density(filename, hv):
    r"""
    This function is called by the qmt.visualizer.plot function - it is
    responsible for generating the Holoviews plots
    """
    # First, read in the relevant data
    data_file = h5py.File(filename, 'r')
    kdims = data_file['list_of_tags']
    points = data_file['tagged_value_list']
    densities = [data_file[str(index) + '_rho']
                 for index in range(len(points))]
    density_units = [data_file[str(index) + '_rho_units'][()]
                     for index in range(len(points))]
    meshes = [data_file[str(index) + '_mesh'] for index in range(len(points))]
    mesh_units = [data_file[str(index) + '_mesh_units'][()]
                  for index in range(len(points))]
    ...
    def density_plot(index):
        ...
        # This plots the electrons and holes on top of one another in a log
        # scale
        im1 = hv.Image((xs, ys, (np.transpose(density_data) < -
                                 1.e16) *
                        (abs(np.transpose(density_data)))),
                       kdims=[('x', 'x (' +
                               str(mesh_units[index]) +
                               ')'), ('y', 'y (' +
                                      str(mesh_units[index]) +
                                      ')')],
                       vdims=[('density', 'electron density (e/' +
                               str(density_units[index]) +
                               r'$^3$)')],
                       group='electrons').options(clims=(1.e16, 1.e19),
                                                  logz=True)

        im2 = hv.Image((xs, ys, (np.transpose(density_data) > 1.e16) *
                        (abs(np.transpose(density_data)))),
                       kdims=[('x', 'x (' +
                               str(mesh_units[index]) +
                               ')'), ('y', 'y (' +
                                      str(mesh_units[index]) +
                                      ')')],
                       vdims=[('density', 'hole density (e/' +
                               str(density_units[index]) +
                               r'$^3$)')],
                       group='holes').options(clims=(1.e16, 1.e19),
                                              logz=True)
        hv.opts("Image.electrons (cmap='Reds') "
                "[colorbar=True,cbar_padding=0]")
        hv.opts(
            "Image.holes (cmap='Blues') "
            "[colorbar=True,cbar_padding=0.1,title_format='Charge density']")
        return im1 * im2
    plot_dict = {tuple(points[index]): density_plot(index)
                 for index in range(len(points))}
