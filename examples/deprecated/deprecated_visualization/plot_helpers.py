...
def save_relevant_data(
        generic_task,
        filename,
        dask_client,
        get_relevant_data,
        plot_type):
    # sweepholder containing list of futures
    sweep_holder = generic_task.computed_result
    data = list(map(lambda x: dask.delayed(
        get_relevant_data)(x), sweep_holder.futures))
    # list of futures pointing to relevant data
    data = list(map(lambda x: dask_client.compute(x), data))
    with h5py.File(filename, 'w') as data_file:
        used = set()
        unique_tags = [
            x for x in sweep_holder.sweep.list_of_tags if x not in used and (
                used.add(x) or True)]
        print(str(unique_tags))
        dt = h5py.special_dtype(vlen=str)
        data_file.create_dataset('list_of_tags', data=list(
            map(lambda x: str(x).encode('utf8'), unique_tags)), dtype=dt)
        points = []
