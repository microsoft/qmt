...
def save_relevant_data(
    generic_task, filename, dask_client, get_relevant_data, plot_type
):
    sweep_holder = (
        generic_task.computed_result
    )  # sweepholder containing list of futures
    data = list(map(lambda x: dask.delayed(get_relevant_data)(x), sweep_holder.futures))
    data = list(
        map(lambda x: dask_client.compute(x), data)
    )  # list of futures pointing to relevant data
    with h5py.File(filename, "w") as data_file:
        used = set()
        unique_tags = [
            x
            for x in sweep_holder.sweep.list_of_tags
        ...
        data_file.create_dataset(
            "list_of_tags",
            data=list(map(lambda x: str(x).encode("utf8"), unique_tags)),
            dtype=dt,
        )
        points = []
