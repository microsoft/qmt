...
def generate_mobility_plots(
        ...
        dask_client=None):
    r"""
    This function takes as input a mobility task, a filename, the tag which is
    the x axis of the eventual plot, and the units of the x-axis as a string.
    ...
    def _get_relevant_data(mobility_data):
        ...
        output['conductance_units'] = \
            mobility_data.content['conductance_units']
...
def _plot_mobility(filename, hv):
    ...
    conductance = [
        data_file[str(index) + '_conductance'].value
        for index in range(len(points))]
    conductance_units = [data_file[str(
        index) + '_conductance_units'].value
        for index in range(len(points))]
    mobility = [
        data_file[str(index) + '_mobility'].value
        for index in range(len(points))]
    mobility_units = [
        data_file[str(index) + '_mobility_units'].value
        for index in range(len(points))]
    ...
    new_data = [[], [], []]
    x_axis_index = list(
        map(lambda x: x.encode('utf8'), kdims)).index(x_axis_label)
    ...
    def mobility_plots(index):
        ...
        + hv.Curve((x_data,
                   ...
                   kdims=[('x',
                           ...
                           ' (' + x_axis_units.decode('utf8') + ')'),
                          ('mobility',
                           'mobility (' + str(mobility_units[index]) + ')')])
    curve_dict_2D = {
        tuple(
            points[index]): mobility_plots(index) for index in range(
            len(points))}
