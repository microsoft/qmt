...
def _plot_1d_density(filename, hv):
    ...
    densities = [data_file[str(index) + '_densities']
                 for index in range(len(points))]
    density_units = [data_file[str(index) + '_density_units'][()]
                     for index in range(len(points))]
    ...
    def density_plot(index):
        ...
        return hv.Curve((mesh_data,
                         density_data),
                        kdims=[('x',
                                'x (' + str(mesh_units[index]) + ')'),
                               ('density',
                                'density (' + str(density_units[index]) +
                                ')')])
    curve_dict_2D = {
        tuple(
            points[index]): density_plot(index) for index in range(
            len(points))}
