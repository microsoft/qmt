...
class MeshingTaskExample(Task):
    def __init__(self, geo_task, options=None, name="meshing_task", gather=True):
        super(MeshingTaskExample, self).__init__(
            [geo_task], options, name, gather=gather
        )
    ...
    def _solve_gathered(self, list_of_input_result_lists, list_of_options):
        ...
        reduced_sweep = qtf.ReducedSweep.create_from_manager_and_tags(
            self.sweep_manager, self.list_of_tags
        )
        new_delayed_result = qtf.ReducedSweepDelayed.create_from_reduced_sweep_and_manager(
            reduced_sweep, self.sweep_manager
        )
        for sweep_holder_index, tag_values in enumerate(
            new_delayed_result.tagged_value_list
        ):
...
geo_dict = {"part1": {"side length": tag1}, "part2": {"side length": 3.0}}
geo_task = GeometryTaskExample(options=geo_dict)

mesh_dict = {"part1": {"min_size": 1.0}, "part2": {"min_size": tag2}}
mesh_task = MeshingTaskExample(geo_task, options=mesh_dict)

poisson_dict = {"part1": {"voltage": tag3}, "part2": {"voltage": 1.0}}
poisson_task = PoissonTaskExample(mesh_task, options=poisson_dict)

sweeps = [
    {tag1: 1.0, tag2: 10.0, tag3: 1.0},
    {tag1: 2.0, tag2: 10.0, tag3: 1.0},
    {tag1: 1.0, tag2: 5.0, tag3: 1.0},
    {tag1: 4.0, tag2: 3.0, tag3: 2.0},
]
