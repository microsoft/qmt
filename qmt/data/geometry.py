...
class Geo2DData(object):
    """
    Class for holding a 2D geometry specification. This class holds two main
    dicts:
        - parts is a dictionary of shapely Polygon objects
        - edges is a dictionary of shapely LineString objects

    Parts are intended to be 2D domains, while edges are used for setting
    boundary conditions and surface conditions.
    """
    ...
    def add_part(self, part_name, part, overwrite=False):
        ...
        :param Polygon part: Polygon object from shapely.geometry. This must
            be a valid Polygon.
        ...
        if (part_name in self.parts) and (not overwrite):
            raise ValueError(
                "Attempted to overwrite the part " +
                part_name +
                ".")
        else:
    ...
    def remove_part(self, part_name, ignore_if_absent=False):
        ...
        :param bool ignore_if_absent: Should we ignore an attempted removal
    ...
    def add_edge(self, edge_name, edge, overwrite=False):
        ...
        if (edge_name in self.edges) and (not overwrite):
            raise ValueError(
                "Attempted to overwrite the edge " +
                edge_name +
                ".")
        else:
    ...
    def remove_edge(self, edge_name, ignore_if_absent=False):
        ...
        :param bool ignore_if_absent: Should we ignore an attempted removal if
            the part name is not found?
        """
    ...
    def compute_bb(self):
        """
        Computes the bounding box of all of the parts and edges in the
        geometry.
    ...
    def part_coord_list(self, part_name):
        ...
        :return list coord_list: List of coordinates of the vertices of the
                                 part.
        """
        ...
        coord_list = list(
            np.array(
                self.parts[part_name].exterior.coords.xy).T)[
            :-1]
        return coord_list
...
class Geo3DData(object):
    ...
    Class for a 3D geometry specification. It holds:
        - parts is a dict of Part3D objects, keyed by the label of each Part3D
          object.
    ...
    def __init__(self):
        ...
        # serialized FreeCAD document for this geometry
        self.serial_fcdoc = None
        # numpy array corresponding to the mesh vertices
        ...
        self.mesh_regions = None
        # dictionary with part name keys mapping to region IDs
        self.mesh_id_dict = None
        # Dict with 1D arrays; each entry specifies what tets belong to
        # virtual region
    ...
    def add_xsec(self, xsec_name, polygons, axis=(1., 0., 0.), distance=0.):
        """
        Make a cross-section of the geometry perpendicular to the axis at a
        given distance from the origin.
        :param str xsec_name: a strong giving the name for the cross section.
        :param dict polygons: dict conrresponding to the cross-section
            polygons.
        :param tuple axis: Tuple defining the axis that defines the normal of
            the cross section.
        :param float distance: Distance along the axis used to set the cross
            section.
        """
    ...
    def set_data(self, data_name, data, scratch_dir=None):
        ...
        :param scratch_dir: Optional existing temporary (fast) storage
            location.
        """
        if data_name == 'fcdoc':
    ...
    def get_data(self, data_name, mesh=None, scratch_dir=None):
        ...
        :param str data_name: "fcdoc" freeCAD document.
        :param scratch_dir: Optional existing temporary (fast)
            storagelocation.
...
class Part3DData(object):
    ...
    :param str fc_name: The name of the 2D/3D freeCAD object that this is
        built from. Note that if the label used for the 3D part is the same as
        the freeCAD label, and that label is unique, None may be used here as
        a shortcut.
    :param str directive: The freeCAD directive is used to construct this
        part.
    ...
    :param str domain_type: The type of domain this part represents. Valid
        options are:
    ...
    :param Part3DData target_wire: Target wire directive for a coating
        directive.
    :param list shell_verts: Vertices to use when rendering the coating.
        Required for the shell directive.
    :param str depo_mode: 'depo' or 'etch' defines the positive or negative
        mask for the deposition of a wire coating.
    :param float z_middle: The location for the "flare out" of the SAG
        directive.
    :param float t_in: The lateral distance from the 2D profile to the edge of
        the top bevel for the SAG directive.
    :param float t_out: The lateral distance from the 2D profile to the
        furthest "flare out" location for the SAG directive.
    :param int layer_num: The layer number used by the lithography directive.
        Lower numbers go down first, with higher numbers deposited last.
    :param list litho_base: The base partNames to use for the lithography
        directive. For multi-step lithography, the bases are just all merged,
        so there is no need to list this more than once.
    :param float mesh_max_size: The maximum allowable mesh size for this part,
        in microns.
    :param float mesh_min_size: The minimum allowable mesh size for this part,
        in microns.
    :param float mesh_growth_rate: The maximum allowable mesh growth rate for
        this part.
    :param tuple mesh_scale_vector: 3D list with scaling factors for the mesh
        in x, y, z direction.
    :param dict boundary_condition: One or more boundary conditions, if
        applicable, of the form of a type -> value mapping. For example, this
        could be {'voltage':1.0} or, more explicitly, {'voltage': {'type':
        'dirichlet', 'value': 1.0,'unit': 'V'}}. Assumed by FEniCS solvers to
        be in the form {"voltage":1.0}, and the value given is taken to be in
        meV.
    :param list subtract_list: A list of partNames that should be subtracted
        from the current part when forming the final 3D objects. This
        subtraction is carried out when boundary conditions are set.
    :param float ns: Volume charge density of a part, applicable to
        semiconductor and dielectric parts. The units for this are 1/cm^3.
    :param float phi_nl: The neutral level for interface traps, measured in
        units of eV above the valence band maximum (semiconductor only).
    ...
    def __init__(
        ...
        if domain_type not in [
            "semiconductor",
                "dielectric"] and ns is not None:
            raise ValueError(
                "Cannot set a volume charge density on a gate or virtual"
                "part.")
