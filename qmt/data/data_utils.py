...
def store_serial(obj, save_fct, ext_format, scratch_dir=None):
    ...
    The parameter `ext_format` can be used for format distinction in some
    `save_fct`.
    '''
    if not scratch_dir:
...
def load_serial(serial_obj, load_fct, ext_format=None, scratch_dir=None):
    ...
    The `load_fct` must be a correct complement of the previously used
    `store_fct`.
    '''
    if not ext_format:
        ext_format = 'tmpdata'
...
def reduce_data(reduce_function, task, dask_client):
    """
    Given a task that has or will be been run, apply a reduce function to all
    of its outputs in dask. By specifying a custom `reduce_function`, the user
    is returning exactly what they want from a given run.

    :param function reduce_function: A function that takes the output data
                                     type of the supplied task and returns a
                                     dictionary of objects that can be stored
                                     in hdf5.
    :param Task task: The task that we would like to work on. Note that this
                      function doesn't run the task, but this can be set up
                      either before or after running.
    :param dask_client: The client we are using for the calculation
    :return sweep_vals,extracted_data: Returns a list of the sweep tags and a
                                       list of the futures corresponding to
                                       the data objects.
    """
    # List of futures that resolve to the data
    ...
    # Next, send these futures to the client to perform the reduction
    # remotely:
...
def stream_data_to_file(
        ...
        sweep_vals=None):
    """
    Instead of simply retrieving all the data, we can stream it to a file on
    disk as the runs complete. The data are stored in an hdf5 file with a
    single level. Data entries are given by keys of the form "index_paramval",
    where index is the numerical index of the result in the extracted_data
    list and paramval is the descriptive key for the datum of interest.

    :param extracted_data: List of futures we ant to retrieve.
    :param filename: File name for the local data store.
    :param dask_client: The client we are using for the calculation
    :param sweep_vals: Sweep point values to store along with the data. If
                       None, then just uses an integer list.
    """
