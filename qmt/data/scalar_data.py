...
class FenicsPotentialData3D(InterpolatableScalarData3D):
    def __init__(self, fenics_potential, solver_input, coords_units, data_units, inf_norm_inconsistency=None):
        ...
        self.charge_density, self.parts_to_volume_charge_integrals = self._initialize_non_surface_charge_density(
            solver_input)
        self.parts_to_boundary_densities, self.parts_to_boundary_charge_integrals = self._initialize_surface_densities(
            solver_input)
...
class FenicsThomasFermiData3D(FenicsPotentialData3D):
    ...
    def _initialize_non_surface_charge_density(self, solver_input):
        ...
        for part in solver_input.volume_charge_parts():
            integrand = fn.Constant(0.0) * measure

            if part in solver_input.parts_to_volume_charges:
                ns = solver_input.parts_to_volume_charges[part]
                region_id = names_to_ids[part]
                integrand += fn.Constant(ns) * measure(region_id)

            if part in solver_input.accumulation_parts():
                known_potential = self.phi
                region_id = names_to_ids[part]
                integrand += make_accumulation_term(solver_input, known_potential) * measure(region_id)

            volume_charge_integrals[part] = fn.assemble(integrand)

        return None, volume_charge_integrals
    ...
    def _initialize_surface_densities(self, solver_input):
        ...
        for part in solver_input.surface_charge_parts():
            ...
            integrand = fn.Constant(0.0) * measure[part]

            if part in solver_input.parts_to_neumann_bcs:
                charge_density = solver_input.parts_to_neumann_bcs[part]
                integrand += fn.Constant(charge_density) * measure[part](1)

            if part in solver_input.parts_to_phi_nl_and_ds:
                known_potential = self.phi
                integrand += get_raw_dit_term(part, solver_input.parts_to_phi_nl_and_ds[part], solver_input,
                                              self.solver_options, known_potential) * measure[part]

            surface_charge_integrals[part] = fn.assemble(integrand)

        return None, surface_charge_integrals
...
def make_accumulation_term(solver_input, known_potential):
    ...
    return -prefactor_expression * abs(known_potential - solver_input.reference_level - u0_expression) ** (3/2) * fn.conditional(
