...
class PropertyMap(object):
    """
    Map points in the simulation domain to properties of parts containing the
    points.
    ...
    def get_part(self, x):
        ...
        :return: Part identifier (if x is a single coordinate vector); or
            array of part identifiers, of the same shape as x except for the
    ...
    def __call__(self, x):
        ...
        :param x: Coordinate vector or array of coordinate vectors.
        :return: Property of the part(s) containing `x`, of the same shape as
            `x` except for the last axis corresponding to coordinate vector
            extent.
        """
...
class MaterialPropertyMap(PropertyMap):
    """
    Map points in the simulation domain to material properties of parts
    containing the points.

    :param PartMap part_map: Function that takes a spatial location and maps
        it to a part identifier.
    :param dict part_materials: Dict mapping from part identifier to a
        material name.
    :param qmt.Materials mat_lib: Materials library used to look up the
        material properties.
    :param str prop_name: Name of the material property to be retrieved for
        each part.
    :param eunit: Energy unit, passed to `mat_lib.find()`.
    :param fill_value: Value to be filled in places where there is no part or
        the part does not have a material or the material does not have the
        property `prop_name`. The default behavior `fill_value='raise'` is to
        raise a KeyError in these cases.
    """
